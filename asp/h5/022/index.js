var Fireworks = function(){
    ...
    self.dt = 0;
		...
		self.canvas.width = self.cw = 1800;
		self.canvas.height = self.ch = 3000;
	...
	var Particle = function(x, y, hue){
		...
		// 调节烟花大小
		// TODO this.speed = rand(((self.partSpeed - self.partSpeedVariance) <= 0) ? 1 : self.partSpeed - self.partSpeedVariance, (self.partSpeed + self.partSpeedVariance));
		  this.speed = 35;
	...
	var Firework = function(startX, startY, targetX, targetY){
		...
		this.shockwaveAngle = Math.atan2(targetY - startY, targetX - startX)+(90*(Math.PI/100));
		this.acceleration = self.fworkAccel/100;
		this.hue = self.currentHue;
		this.brightness = rand(500, 80);
	...
	Firework.prototype.update = function(index){
		...
		// 调节发射速度 
		// TODO this.speed *= 1 + this.acceleration;
		this.speed = 10; 
		this.coordLast[2].x = this.coordLast[1].x;
		this.coordLast[2].y = this.coordLast[1].y;
		this.coordLast[1].x = this.coordLast[0].x;
	...
	Firework.prototype.draw = function(){
		...
		if(self.showShockwave){
			...
			self.ctx.beginPath();
			//TODO self.ctx.arc(0, 0, 1*(this.speed/5), 0, Math.PI, true);
			  self.ctx.arc(0, 0, 5, 0, Math.PI, true);
  ...
  while(initialLaunchCount--){
    setTimeout(function(){
 		self.fireworks.push(new Firework(self.cw/2, self.ch, rand(100, self.cw-100), rand(50, self.ch/2)-50));
    }, initialLaunchCount*300);
  }
...
guiParticles.add(fworks, 'partGravity').min(-20).max(200).step(1);
guiParticles.add(fworks, 'flickerDensity').min(0).max(500).step(1);
...
guiOther.add(fworks, 'clearAlpha').min(0).max(1000).step(1);
